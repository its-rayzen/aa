...
export default function GallerySection() {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      // Next image opacity: stays at 0% until 75%, then goes from 0% to 100% during 75%-100% period
      if (newProgress >= 0.75 && hasTriggeredNext.current) {
        const progressInTransition = (newProgress - 0.75) / 0.25 // 0 to 1 over 25% period
        setNextImageOpacity(progressInTransition) // 0 to 1
        
        // When transition completes (at 100%), next image should be at 100% opacity
        // This ensures continuity when it becomes the current image
        nextImageInheritedOpacity.current = progressInTransition
      }
